...
/*   Updated: 2022/01/04 21:33:17 by student42ab      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

int	ft_init(int fd, char **tail, char **buf)
{
...
void	ft_free(char **buf, char **tail)
{
...
int	ft_check_nl(int nl, char **buf, char **tail)
{
	if (nl == -1 && (*tail)[0])
...
{	
	...
	if (nl == -1)
		return (NULL);
	next_line = ft_substr(tail[fd], 0, nl + 1);
